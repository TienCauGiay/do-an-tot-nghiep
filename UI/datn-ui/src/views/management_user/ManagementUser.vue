<template>
  ...
  <div class="content-main-body">
    <div class="content-action">
      ...
      <div class="search-entity">
        <input
          ...
          @input="autoSearch"
        />
        <div class="search-icon icon-tb" @click="onSearchUser"></div>
      </div>
      <div
        @click="refreshData"
    ...
    <div id="list-user" class="list-entity">
        ...
        <table id="tbUserList">
          <thead>
            <tr>
              ...
              <th>
                {{ this.$_MSResource[this.$_LANG_CODE].User_Column.Status }}
              </th>
              <th type="feat" class="text-center entity-border-right e-birthday">
                {{ this.$_MSResource[this.$_LANG_CODE].User_Column.Feature }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr
              ...
              :class="{ checkedRow: checkRow().includes(item.user_id) }"
            >
              <td class="entity-border-left" @dblclick.stop>
                <div class="th-checkbox">
                  <input
                    ...
                    type="checkbox"
                    @click="checkRow(item.user_id)"
                    :checked="checkRow().includes(item.user_id)"
                  />
                </div>
              </td>
              ...
              <td
                :title="
                  item.status == 1
                    ? $_MSResource[this.$_LANG_CODE].FORM.Active
                    : $_MSResource[this.$_LANG_CODE].FORM.InActive
                "
              >
                {{
                  item.status == 1
      ...
      <teleport to="#app">
        ...
        >
          ...
          <div @click="resetPassWork">
            {{ this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.resetPassWork }}
          </div>
        </div>
      </teleport>
      <img
    ...
    <ms-dialog-confirm-delete
      ...
      }${userNameDeleteSelected}${this.$_MSResource[this.$_LANG_CODE].DIALOG.CONTENT.END}`"
...
export default {
  ...
  created() {
    ...
    this.$_MSEmitter.on("confirmDeleteEntity", async () => {
      await this.btnConfirmDeleteUser();
    });
    this.$_MSEmitter.on("unConfirmDeleteEntity", () => {
      this.btnUnConfirmDeleteUser();
    });
    this.$_MSEmitter.on("confirmDeleteMultiple", async () => {
      await this.btnconfirmDeleteMultipleUser();
    });
  ...
  data() {
    return {
      ...
      userIdDeleteMultiple: "",
      // Khai báo user của sinh viên cần xóa
      userNameDeleteSelected: "",
  ...
  computed: {
    ...
    isCheckAll() {
      ...
      for (let i = 0; i < this.dataTable.Data.length; i++) {
        if (!this.ids.includes(this.dataTable.Data[i].user_id)) {
          return false;
        }
      }
      return true;
  ...
  methods: {
    ...
    async getDataUser() {
      try {
        ...
        const resfilter = await userService.getFilter(this.selectedRecord, this.currentPage, "");
    ...
    onDeleteUser() {
      ...
      this.isOverlay = true;
      this.userIdDeleteMultiple = this.selectedUser.user_id;
    },
    /**
     ...
     */
    async btnConfirmDeleteUser() {
      try {
        this.isShowLoading = true;
        const res = await userService.delete(this.userIdDeleteMultiple);
        this.isShowLoading = false;
    ...
    async onSearchUser() {
      try {
        ...
        this.isShowLoading = true;
        const filteredUsers = await userService.getFilter(
          this.selectedRecord,
          this.currentPage,
    ...
    async updateDataTable() {
      try {
        ...
        this.isShowLoading = true;
        const resfilter = await userService.getFilter(this.selectedRecord, this.currentPage, this.textSearch.trim());
        this.isShowLoading = false;
        this.dataTable = resfilter.data;
      } catch {
        return;
      ...
      try {
        if (this.isCheckAll) {
          this.dataTable.Data.map((item) => {
            if (this.ids.includes(item.user_id)) {
              this.ids.splice(this.ids.indexOf(item.user_id), 1);
            }
          });
        } else {
     ...
     */
    async btnconfirmDeleteMultipleUser() {
      try {
        this.isShowLoading = true;
        const res = await userService.deleteMutiple(this.ids);
        this.isShowLoading = false;
        this.isShowDialogConfirmDelete = false;
        this.isOverlay = false;
    ...
    /**
     * Mô tả: Đặt lại mật khẩu mặc định cho tài khoản
     * created by : BNTIEN
     * created date: 13-03-2024 22:34:41
     */
    async resetPassWork() {
      try {
        this.isShowLoading = true;
        const res = await userService.resetPassWork(this.selectedUser.user_id);
        this.isShowLoading = false;
        if (res && this.$_MSEnum.CHECK_STATUS.isResponseStatusOk(res.Code) && res.Data) {
          this.contentToastSuccess = this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.resetPassWorkSuccess;
          this.onShowToastMessage();
          await this.getDataUser();
        }
      } catch {
