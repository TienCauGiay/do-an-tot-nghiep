<template>
  ...
  >
    ...
    <ms-dialog-data-exist
      ...
      :textProp="this.$_MSResource[this.$_LANG_CODE].DIALOG.CONTENT.UserName"
...
export default {
  ...
  methods: {
    ...
    async checkExists() {
      try {
        const res = await userService.getByUserName(this.user.user_name);
        return res.data;
      } catch {
        return null;
      }
      ...
      if (this.statusFormMode === this.$_MSEnum.FORM_MODE.Add) {
        ...
        if (this.hasDataChanged()) {
          ...
          if (this.dataNotNull.length > 0) {
            ...
            try {
              let codeExist = (await this.checkExists()).Data;
              if (!codeExist || codeExist.user_name === this.user.user_name) {
    ...
    handleExisted(itemExist) {
      ...
      this.isBorderRed.user_name = true;
      this.errors["user_name"] = `${this.$_MSResource[this.$_LANG_CODE].DIALOG.CONTENT.UserName}
       ${itemExist.user_name} ${this.$_MSResource[this.$_LANG_CODE].DIALOG.CONTENT.EXIST_DETAIL_END}`;
      this.contentCodeExist = itemExist.user_name;
    },
    /**
