<template>
  <div class="content-title">
    <h1>{{ this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.ManagementSubject }}</h1>
  </div>
    ...
    <div class="content-action">
      ...
      <div class="search-entity" title="Tìm kiếm theo mã, tên môn học">
        <input
          ...
          v-model="textSearch"
          @keydown.enter="onSearchSubject"
          @input="autoSearch"
        />
        <div class="search-icon icon-tb" @click="onSearchSubject"></div>
      </div>
      <div
        @click="refreshData"
        ...
        <table id="tbUserList">
          ...
          <tbody>
            <tr
              ...
              :class="{ checkedRow: checkRow().includes(item.subject_id) }"
            >
              <td class="entity-border-left" @dblclick.stop>
                <div class="th-checkbox">
                  <input
                    ...
                    type="checkbox"
                    @click="checkRow(item.subject_id)"
                    :checked="checkRow().includes(item.subject_id)"
                  />
                </div>
              </td>
...
export default {
  ...
  created() {
    ...
    this.$_MSEmitter.on("confirmDeleteEntity", async () => {
      await this.btnConfirmDeleteSubject();
    });
    this.$_MSEmitter.on("unConfirmDeleteEntity", () => {
      this.btnUnConfirmDeleteSubject();
    });
    this.$_MSEmitter.on("confirmDeleteMultiple", async () => {
      await this.btnconfirmDeleteMultipleSubject();
    });
  ...
  computed: {
    ...
    isCheckAll() {
      ...
      for (let i = 0; i < this.dataTable.Data.length; i++) {
        if (!this.ids.includes(this.dataTable.Data[i].subject_id)) {
          return false;
        }
      }
      return true;
  ...
  methods: {
    ...
    async getDataService() {
      try {
        ...
        const resfilter = await subjectService.getFilter(this.selectedRecord, this.currentPage, "");
    ...
    onDeleteSubject() {
      ...
      this.isOverlay = true;
      this.subjectIdDelete = this.selectedSubject.subject_id;
    },
    async checkArise() {
      try {
        const res = await subjectService.checkArise(this.subjectIdDelete);
    ...
    async btnConfirmDeleteSubject() {
      try {
        if (await this.checkArise()) {
          ...
          this.isShowLoading = true;
          const res = await subjectService.delete(this.subjectIdDelete);
          this.isShowLoading = false;
          this.isShowDialogConfirmDelete = false;
          this.isOverlay = false;
    ...
    async onSearchSubject() {
      try {
        ...
        this.isShowLoading = true;
        const filteredUsers = await subjectService.getFilter(
          this.selectedRecord,
          this.currentPage,
    ...
    async updateDataTable() {
      try {
        ...
        this.isShowLoading = true;
        const resfilter = await subjectService.getFilter(this.selectedRecord, this.currentPage, this.textSearch.trim());
        this.isShowLoading = false;
        this.dataTable = resfilter.data;
      } catch {
        return;
      ...
      try {
        if (this.isCheckAll) {
          this.dataTable.Data.map((item) => {
            if (this.ids.includes(item.subject_id)) {
              this.ids.splice(this.ids.indexOf(item.subject_id), 1);
            }
          });
        } else {
    ...
    async getIdArise() {
      try {
        const res = await subjectService.getIdArise(this.ids);
        return res?.Data;
      } catch (error) {
        console.log(error);
        return [];
     ...
     */
    async btnconfirmDeleteMultipleSubject() {
      try {
        this.isShowDialogConfirmDelete = false;
        this.isOverlay = false;
        this.isShowLoading = true;
        ...
        if (listIdArise && listIdArise.length > 0) {
          ...
          let listSubjectName = this.dataTable.Data.filter((x) => listIdArise.includes(x.subject_id)).map(
            (x) => x.subject_name
          );
          let messageWarning = listSubjectName.join(", ");
          ...
          this.isShowLoading = true;
          const res = await subjectService.deleteMutiple(this.ids);
          this.isShowLoading = false;
    ...
    async btnConfirmDeleteBusiness() {
      try {
        ...
        if (this.listIdNotArise && this.listIdNotArise.length > 0) {
          const res = await subjectService.deleteMutiple(this.listIdNotArise);
          this.isShowLoading = false;
          if (res && res.data && this.$_MSEnum.CHECK_STATUS.isResponseStatusOk(res.data.Code) && res.data.Data) {
            this.ids = [];
            this.listIdNotArise = [];
