<template>
  <div class="content-title">
    <h1>{{ this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.ManagementFaculty }}</h1>
  </div>
  <div class="content-main-body">
    <div class="content-action">
      ...
      <div class="search-entity" title="Tìm kiếm theo mã, tên khoa">
        <input
          type="search"
          :placeholder="this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.PLACEHOLDER_SEARCH_HEADER"
          v-model="textSearch"
          @keydown.enter="onSearchFaculty"
          @input="autoSearch"
        />
        <div class="search-icon icon-tb" @click="onSearchFaculty"></div>
      </div>
      <div
        @click="refreshData"
        ...
        <table id="tbUserList">
          <thead>
            <tr>
              ...
              <th class="w-300">
                {{ this.$_MSResource[this.$_LANG_CODE].Faculty_Column.FacultyCode }}
              </th>
              <th>
                {{ this.$_MSResource[this.$_LANG_CODE].Faculty_Column.FacultyName }}
              </th>
              <th type="feat" class="text-center entity-border-right e-birthday">
                {{ this.$_MSResource[this.$_LANG_CODE].Faculty_Column.Feature }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr
              ...
              :class="{ checkedRow: checkRow().includes(item.faculty_id) }"
            >
              <td class="entity-border-left" @dblclick.stop>
                <div class="th-checkbox">
                  <input
                    ...
                    type="checkbox"
                    @click="checkRow(item.faculty_id)"
                    :checked="checkRow().includes(item.faculty_id)"
                  />
                </div>
              </td>
              <td class="w-300" :title="item.faculty_code">
                {{ item.faculty_code }}
              </td>
              <td :title="item.faculty_name">
                {{ item.faculty_name }}
              </td>
...
export default {
  ...
  created() {
    ...
    this.$_MSEmitter.on("refreshDataTable", async () => {
      await this.getDataFaculty();
    });
    this.$_MSEmitter.on("confirmDeleteEntity", async () => {
      await this.btnConfirmDeleteFaculty();
    });
    this.$_MSEmitter.on("unConfirmDeleteEntity", () => {
      this.btnUnConfirmDeleteFaculty();
    });
    this.$_MSEmitter.on("confirmDeleteMultiple", async () => {
      await this.btnconfirmDeleteMultipleFaculty();
    });
  ...
  computed: {
    ...
    isCheckAll() {
      ...
      for (let i = 0; i < this.dataTable.Data.length; i++) {
        if (!this.ids.includes(this.dataTable.Data[i].faculty_id)) {
          return false;
        }
      }
      return true;
  ...
  methods: {
     ...
     */
    async getDataFaculty() {
      try {
        this.isShowLoading = true;
        const resfilter = await facultyService.getFilter(this.selectedRecord, this.currentPage, "");
    ...
    onDeleteFaculty() {
      ...
      this.isOverlay = true;
      this.facultyIdDelete = this.selectedFaculty.faculty_id;
    },
    async checkArise() {
      try {
        const res = await facultyService.checkArise(this.facultyIdDelete);
    ...
    async btnConfirmDeleteFaculty() {
      try {
        if (await this.checkArise()) {
          ...
          this.isShowLoading = true;
          const res = await facultyService.delete(this.facultyIdDelete);
          this.isShowLoading = false;
          this.isShowDialogConfirmDelete = false;
          this.isOverlay = false;
    ...
    async btnConfirmDeleteBusiness() {
      try {
        ...
        if (this.listIdNotArise && this.listIdNotArise.length > 0) {
          const res = await facultyService.deleteMutiple(this.listIdNotArise);
          this.isShowLoading = false;
    ...
    async onSearchFaculty() {
      try {
        ...
        this.isShowLoading = true;
        const filteredUsers = await facultyService.getFilter(
          this.selectedRecord,
          this.currentPage,
    ...
    async updateDataTable() {
      try {
        ...
        this.isShowLoading = true;
        const resfilter = await facultyService.getFilter(this.selectedRecord, this.currentPage, this.textSearch.trim());
        this.isShowLoading = false;
        this.dataTable = resfilter.data;
      } catch {
        return;
      ...
      try {
        if (this.isCheckAll) {
          this.dataTable.Data.map((item) => {
            if (this.ids.includes(item.faculty_id)) {
              this.ids.splice(this.ids.indexOf(item.faculty_id), 1);
            }
          });
        } else {
    ...
    async getIdArise() {
      try {
        const res = await facultyService.getIdArise(this.ids);
        return res?.Data;
      } catch (error) {
        console.log(error);
        return [];
     ...
     */
    async btnconfirmDeleteMultipleFaculty() {
      try {
        this.isShowDialogConfirmDelete = false;
        this.isOverlay = false;
        this.isShowLoading = true;
        ...
        if (listIdArise && listIdArise.length > 0) {
          ...
          let listFacultyName = this.dataTable.Data.filter((x) => listIdArise.includes(x.faculty_id)).map(
            (x) => x.faculty_name
          );
          let messageWarning = listFacultyName.join(", ");
          ...
          this.isShowLoading = true;
          const res = await facultyService.deleteMutiple(this.ids);
          this.isShowLoading = false;
          if (res && res.data && this.$_MSEnum.CHECK_STATUS.isResponseStatusOk(res.data.Code) && res.data.Data) {
            this.ids = [];
            this.isDeleteMultipleDialog = false;
