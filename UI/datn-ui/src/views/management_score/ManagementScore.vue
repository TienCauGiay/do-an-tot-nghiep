<template>
  <div class="content-title">
    <h1>{{ this.$_MSResource[this.$_LANG_CODE].TEXT_CONTENT.ManagementScore }}</h1>
  </div>
  <div class="content-main-body">
    <div class="content-action">
      ...
      <div class="search-entity">
        <input
          ...
          @input="autoSearch"
        />
        <div class="search-icon icon-tb" @click="onSearchScore"></div>
      </div>
      <div
        @click="refreshData"
        ...
        <table id="tbScoreList">
          ...
          <tbody>
            <tr
              ...
              :class="{ checkedRow: checkRow().includes(item.score_id) }"
            >
              <td class="entity-border-left" @dblclick.stop>
                <div class="th-checkbox">
                  <input
                    ...
                    type="checkbox"
                    @click="checkRow(item.score_id)"
                    :checked="checkRow().includes(item.score_id)"
                  />
                </div>
              </td>
              ...
              <td class="e-bank-account" :title="item.score_attendance">
                {{ item.score_attendance }}
              </td>
              <td class="e-bank-account" :title="item.score_test">
                {{ item.score_test }}
              </td>
              <td class="e-bank-account" :title="item.score_exam">
                {{ item.score_exam }}
              </td>
              <td class="e-bank-account" :title="item.score_average">
                {{ item.score_average }}
              </td>
    ...
    <ScoreDetail
      ...
      :scoreSelected="scoreUpdate"
      :statusFormMode="isStatusFormMode"
    ></ScoreDetail>
    <!-- dialog score confirm delete -->
    <ms-dialog-confirm-delete
      ...
      }${scoreCodeDeleteSelected}${this.$_MSResource[this.$_LANG_CODE].DIALOG.CONTENT.END}`"
...
export default {
  ...
  created() {
    ...
    this.$_MSEmitter.on("confirmDeleteEntity", async () => {
      await this.btnConfirmDeleteScore();
    });
    this.$_MSEmitter.on("unConfirmDeleteEntity", () => {
      this.btnUnConfirmDeleteScore();
    });
    this.$_MSEmitter.on("confirmDeleteMultiple", async () => {
      await this.btnconfirmDeleteMultipleScore();
    });
  ...
  data() {
    return {
      ...
      scoreIdDeleteSelected: "",
      // Khai báo score của sinh viên cần xóa
      scoreCodeDeleteSelected: "",
      ...
      positionFeatureMenu: {},
      // Khai báo biến lưu score khi bấm vào col feature
      selectedScore: {},
  ...
  computed: {
    ...
    isCheckAll() {
      ...
      for (let i = 0; i < this.dataTable.Data.length; i++) {
        if (!this.ids.includes(this.dataTable.Data[i].score_id)) {
          return false;
        }
      }
      return true;
  ...
  methods: {
    ...
    onDeleteScore() {
      ...
      this.isOverlay = true;
      this.scoreIdDeleteSelected = this.selectedScore.score_id;
    },
    /**
     ...
     */
    async btnConfirmDeleteScore() {
      try {
        this.isShowLoading = true;
        const res = await scoreService.delete(this.scoreIdDeleteSelected);
        this.isShowLoading = false;
      ...
      try {
        if (this.isCheckAll) {
          this.dataTable.Data.map((item) => {
            if (this.ids.includes(item.score_id)) {
              this.ids.splice(this.ids.indexOf(item.score_id), 1);
            }
          });
        } else {
     ...
     */
    async btnconfirmDeleteMultipleScore() {
      try {
        this.isShowLoading = true;
        const res = await scoreService.deleteMutiple(this.ids);
        this.isShowLoading = false;
        this.isShowDialogConfirmDelete = false;
        this.isOverlay = false;
