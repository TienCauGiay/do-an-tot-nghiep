<template>
  <div class="dialog-entity position-display-center">
    <div class="title-dialog">
      <h1>
        {{ this.$_MSResource[this.$_LANG_CODE].DIALOG.TITLE.CONFIRM_DELETE }}
      </h1>
      <div
        class="close-icon"
        @click="btnNoConfirmDelete"
        :title="this.$_MSResource[this.$_LANG_CODE].BUTTON.CLOSE"
      ></div>
    </div>
...
export default {
  ...
  methods: {
     ...
     */
    btnConfirmDelete() {
      this.$_MSEmitter.emit("confirmDeleteEntity2");
    },
    ...
    btnConfirmDeleteMultiple() {
      this.$_MSEmitter.emit("confirmDeleteMultiple2");
    },
     ...
     */
    btnNoConfirmDelete() {
      this.$_MSEmitter.emit("unConfirmDeleteEntity2");
    },
  },
};
</script>
